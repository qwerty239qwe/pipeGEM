...
def get_syn_gene_data(model: Union[cobra.Model, Model],
    ...
    data = np.clip(np.concatenate([
        ...
        rng.normal(0, rng.uniform(1, 50), (1, n_sample))) *  # random effect
        rng.exponential(1e4, size=(1, n_sample))  # g length
        for _ in range(n_genes)], axis=0), a_min=0, a_max=None).astype(int)
