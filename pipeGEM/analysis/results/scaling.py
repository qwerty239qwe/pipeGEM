...
class ModelScalingResult(BaseAnalysis):
    ...
    def reverse_scaling(self, model):
        ...
        for ri, rid in enumerate(self._result["rxn_index"]):
            ...
            assert all([mid in met_ids_in_model
                        for mi, mid in enumerate(self._result["met_index"]) if mi in involved_met_ids]), \
                "This method assumes all the metabolites in this model's reactions found in the rescaling step are still in the model."\
                f"Missing mets: {[mid for mi, mid in enumerate(self._result['met_index']) if mi in involved_met_ids if mid not in met_ids_in_model]}"
